...
const DebateInterface: React.FC<DebateInterfaceProps> = ({ session, onUpdateSession, onEndDebate }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Auto-switch to tactics if a fallacy is detected to alert the user
  ...
  return (
    <div className="flex h-full bg-background-dark overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative">
        {/* Header */}
        <div className="h-16 border-b border-gray-800 flex items-center justify-between px-6 bg-surface-darker/80 backdrop-blur-md z-10">
           <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-600">
                <img src={session.thinker.imageUrl} className="w-full h-full object-cover" alt="Thinker" />
              </div>
              <div>
                <h2 className="text-white font-bold text-sm">{session.thinker.name}</h2>
                <div className="flex items-center gap-2">
                    <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                    <p className="text-xs text-accent-gold truncate max-w-[200px]">{session.topic}</p>
                </div>
              </div>
           </div>
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-gradient-to-b from-background-dark to-[#0a0c16]">
            {session.messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-gray-500 opacity-50">
                    <span className="material-icons text-4xl mb-2">forum</span>
                    <p>Make your opening statement to begin.</p>
                </div>
            )}
            ...
            {isLoading && (
                 <div className="flex justify-start">
                    <div className="bg-surface-dark border border-gray-700 rounded-2xl rounded-bl-none p-4 shadow-lg flex gap-2 items-center">
        ...
        {/* Input Area */}
        <div className="p-6 border-t border-gray-800 bg-surface-darker">
            <div className="relative">
                <textarea
                    ...
                    className="w-full bg-surface-dark border border-gray-700 rounded-xl p-4 pr-12 text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary resize-none h-24 custom-scrollbar"
                />
                <button 
                    onClick={handleSend}
            ...
            <p className="text-[10px] text-gray-600 mt-2 text-center">AI responses are generated based on historical texts and may interpret context creatively.</p>
        </div>
      </div>

      {/* Analysis Sidebar (Right Panel) */}
      <div className="w-80 bg-[#0c0e18] border-l border-gray-800 flex flex-col">
        {/* Score Header */}
        ...
        <div className="flex border-b border-gray-800">
            {(['context', 'arguments', 'tactics', 'sources'] as const).map(tab => (
                <button 
                    ...
                    className={`flex-1 py-3 text-[10px] font-bold uppercase tracking-wide transition-colors ${
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
            ...
            {activeTab === 'context' && (
                <div className="space-y-4 animate-fade-in">
                    <div className="relative h-32 rounded-lg overflow-hidden mb-4 border border-gray-700">
                         <img src={session.thinker.imageUrl} className="w-full h-full object-cover" alt="Cover" />
                         <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-3">
                            <span className="text-white font-bold text-lg">{session.thinker.name}</span>
                         </div>
                    </div>
                    <div className="p-4 bg-surface-dark border border-gray-700 rounded-lg">
            ...
            {activeTab === 'arguments' && (
                <div className="space-y-6 animate-fade-in">
                    <div>
                        ...
                        <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg text-sm text-gray-300 leading-relaxed">
                ...
                <div className="space-y-3 animate-fade-in">
                    ...
                    {session.analysis.fallacies.length > 0 ? (
                        session.analysis.fallacies.map((fallacy, idx) => (
                            <div key={idx} className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg animate-slide-in">
                                ...
                                <div className="mb-3">
                                    ...
                                    <p className="text-sm text-gray-300 leading-snug">{fallacy.explanation}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-green-500 uppercase font-bold mb-1">Improvement</p>
                                    <p className="text-sm text-gray-300 leading-snug italic">"{fallacy.improvement}"</p>
                                </div>
                            </div>
                        ))
                    ) : (
                ...
                <div className="space-y-3 animate-fade-in">
                    ...
                    {session.analysis.sources.length > 0 ? (
                         session.analysis.sources.map((source, idx) => (
                            <div key={idx} className="flex items-start gap-3 p-3 bg-surface-dark border border-gray-700 rounded-lg hover:border-accent-gold/50 transition-colors cursor-help group">
